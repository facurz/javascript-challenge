...
export const listPaddockManagerIds = () => {
    return paddockManagers.sort((a, b) => a.id - b.id).map(el => el.id);
};

// 1 Arreglo con los ruts de los responsables de los cuarteles, ordenados por nombre
export const listPaddockManagersByName = () => {
    return paddockManagers.sort((a, b) => a.name.localeCompare(b.name)).map(el => el.taxNumber);
};
...
export const sortPaddockTypeByTotalArea = () => {
    const arrayPaddocksByArea = paddocks.reduce((acc, valorActual) => {
        const elementoYaExiste = acc.find(
            elemento => elemento.paddockTypeId === valorActual.paddockTypeId
        );
        if (elementoYaExiste) {
            return acc.map(elemento => {
                if (elemento.paddockTypeId === valorActual.paddockTypeId) {
                    return {
                        ...elemento,
                        area: elemento.area + valorActual.area,
                    };
                }
                return elemento;
            });
    ...
    const arrayPadocksOrder = arrayPaddocksByArea.sort((a, b) => b.area - a.area).map(item => {
        return paddockType.find(el => el.id === item.paddockTypeId);
    });
    return arrayPadocksOrder.map(el => el.name);
};
...
export const sortFarmManagerByAdminArea = () => {
    ...
    const arrayPaddocksByArea = paddocks.reduce((acc, valorActual) => {
        const elementoYaExiste = acc.find(
            elemento => elemento.paddockManagerId === valorActual.paddockManagerId
        );
        if (elementoYaExiste) {
            return acc.map(elemento => {
                if (elemento.paddockManagerId === valorActual.paddockManagerId) {
                    return {
                        ...elemento,
                        area: elemento.area + valorActual.area,
                    };
    ...
    const arrayPadocksOrder = arrayPaddocksByArea.sort((a, b) => b.area - a.area).map(item => {
        return paddockManagers.find(el => el.id === item.paddockManagerId);
    });
    return arrayPadocksOrder.map(el => el.name);
};
...
export const farmManagerNames = () => {
    // const arrayRutsByName = paddockManagers.sort((a, b) => a.name.localeCompare(b.name)).map(el => el.taxNumber);
    // return arrayRutsByName
